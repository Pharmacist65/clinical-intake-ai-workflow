...
The project models an evidence-linked clinical intake workflow using fictional data:
...
9. A machine-readable capability manifest keeps runtime safety claims visible to the UI and tests.
10. A secondary deterministic rehearsal surface exercises operational gates without assigning clinical meaning.
...
- A Three.js evidence-spine view that visualises workflow stages without representing clinical state or performance
- A sourced UK/US governance explorer that presents implementation review prompts, not compliance determinations
- Deterministic workflow rehearsal with event timelines, replay identifiers and explicit `clinicalMeaning=false`
- A runtime capability manifest showing disabled clinical and integration capabilities

The frontend depends on the `ClinicalIntakeApi` interface. In normal local mode, `frontend/src/api.ts` calls the backend; the API base URL defaults to `http://localhost:5108` and can be overridden with `VITE_API_BASE_URL`. In Vite `demo` mode, the same interface is implemented by `frontend/src/demoApi.ts` using in-memory fictional data. Static-mode changes reset on reload and never leave the browser.
...
- System capability metadata is exposed at `GET /api/system/capabilities` and covered by an integration test.

### Browser-Only Showcase

`npm run build:demo` creates the static showcase published through GitHub Pages. It contains no ASP.NET backend, database, external AI provider, analytics or live integration. Its in-memory adapter supports the public UI workflow with fictional data so architecture reviewers can inspect interactions without local backend setup.

Frontend changes on reviewed `main` commits trigger the Pages workflow automatically; `workflow_dispatch` also supports an explicit rebuild. The deployment boundary is documented in [static-demo-and-github-pages.md](static-demo-and-github-pages.md).

Vitest contract tests cover the static capability boundary, seeded review queue and fictional create-summary-review lifecycle. CI runs these tests before both frontend build modes.
...
The frontend includes a secondary deterministic rehearsal surface for provenance gates, low-confidence handoffs and medication documentation prompts. Each run exposes an ordered event log, stable replay identifier and debrief. Its evaluation scope is operational controls only; it does not simulate clinical reality or score clinical decisions.

See [workflow-rehearsal-concept.md](workflow-rehearsal-concept.md) for the current contract and future assurance direction.
